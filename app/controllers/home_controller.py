...
def tweets_to_label(user_id):
    ...
    app.logger.info("t_labeled_by: " + str(tweets_ids_labeled_by))
    if tweets_ids_labeled_by:
        tweets_to_label = Tweet.query.filter(Tweet.id.notin_(tweets)).all()
        app.logger.info("t_labeled_by not in: " + str(len(tweets_to_label)))
    else:
